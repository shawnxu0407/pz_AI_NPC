...
操作：游戏内按 ]（右方括号键），会在玩家附近生成一个僵尸，并给它打上 IsCompanion 标记。
（原本先后试过 F9、F7，都被 debug 模式下的其他功能占用了，换成不太会被功能键
 冲突的普通符号键 ] 。如果这个还冲突，可以换成别的不常用键，比如 ; 或者 \。）
...
print("[CompanionZombie] Test script loaded. Press ] in-game to spawn.")
